...
class WallStopTest(unittest.TestCase):
    def set_and_get(self,lf,ls,rs,rf):
        with open("/def/rtlihgtsensor0","w") as f:
            f.write("%d %d %d %d\n" % (rf,rs,ls,lf))

        time.sleep(0.3)

        with open("/dev/rtmotor_raw10","r") as lf,\
             open("/dev/rtmotor_rawr0","r") as rf:
            left = int(lf.readline().rstrip())
            right = int(rt.readline().rstrip())
...
if __name__ == '__main__':
    ...
    rostest.tosrun('pimouse_run_corridor', 'travis_test_wall_stop',WallStopTest)
